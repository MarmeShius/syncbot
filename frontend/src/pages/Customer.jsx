...
function Customer() {
  ...
  return <div className="min-h-screen bg-slate-50 pb-12">
    <header className="border-b bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-indigo-600">SyncBot / Customer Portal</p><h1 className="mt-1 text-xl font-bold">Welcome back, {user?.name || "Customer"}</h1></div><span className="rounded-full bg-indigo-100 px-4 py-2 text-sm font-semibold text-indigo-700">{user?.email}</span></div></header>
    <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10"><div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-medium text-indigo-600">Customer Dashboard</p><h2 className="mt-1 text-3xl font-bold">How can we help you?</h2><p className="mt-2 text-sm text-slate-500">Create and track support requests with a complete conversation history.</p></div><button onClick={() => setView("create")} className="rounded-lg bg-indigo-600 px-5 py-3 text-sm font-semibold text-white">+ Create New Ticket</button></div>
